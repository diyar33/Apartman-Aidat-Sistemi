                        ...
                        <div class="portlet-body">
                            ...
                            <div class="portlet-body">
                                ...
                                <table  class="table table-striped table-bordered table-hover" id="sample_6">
                                    ...
                                    <?php
                                    $sql = mysqli_query($b," SELECT * FROM sikayet WHERE yetki='0' and islem='ÜCRETLİ' order by apt");
$sayac=1;
                                    while($row = mysqli_fetch_array($sql)) {
                                        $id = $row["id"];
                                        $par2=mysqli_fetch_array(mysqli_query($b,"select * from taksit where sikayetid='$id'"));
                                        $sql2=mysqli_query($b,"select * from odemeyer where sikayetid='$id'");
                                        $taksitsay=$par2["taksitsay"];
...
echo '<td>';
                                        if ($taksitsay>1){
                                            for ($i=1;$i<=$taksitsay;$i++){
                                                echo '<pre>'.$i.'.taksit='.$par2["taksit$i"].'&nbsp;TL&emsp;  Ödeme Tarihi='.$par2["tarih$i"].'</pre>';
                                            }
                                        }
                                        else{
...
echo '<td>';
                                        while ($par3=mysqli_fetch_array($sql2))
                                        {
                                            echo "<pre>Ödeme Yeri :".$par3["odemeyer"]."<br>Ödeme Tarih: ".$par3["odeme_tarih"]."  <br>Ödemnecek tutar: ".$par3["fiyat"]."  TL </pre>";
                                        }
echo'</td>';
$yazilacakyer=explode('_',$par2["yazilacakyer"]);
echo '
<td>
<span class="badge bg-blue">
'.$yazilacakyer[0].'&nbsp;'.$yazilacakyer[1].'
</span>
</td>
';
echo '
